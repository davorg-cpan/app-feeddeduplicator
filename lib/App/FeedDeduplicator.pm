...
App::FeedDeduplicator is a simple Perl application that aggregates,
deduplicates, and publishes feeds. It uses the LWP::UserAgent module to
fetch feeds and XML::Feed parse them.

The deduplication process is based on the entry's unique identifier, the
canonical link or the title of the entry.
...
a JSON configuration file. The configuration file should specify the feeds to
be aggregated, the output format, and the maximum number of entries to include
in the output.

The application is designed to be extensible, allowing for additional features
and functionality to be added in the future.

=head1 CONFIGURATION

The configuration file is a JSON file that specifies the feeds to be
aggregated, the output format, and the maximum number of entries to include
in the output.
...
Creates a new instance of App::FeedDeduplicator. The constructor accepts a
configuration file path as an optional parameter.

If the configuration file is not provided, it defaults to
C<~/.feed-deduplicator/config.json> or the path specified by the
C<FEED_DEDUP_CONFIG> environment variable.

=head2 run

The main method that runs the application. It aggregates feeds, deduplicates
entries, and publishes the output.

It creates instances of the Aggregator, Deduplicator, and Publisher classes
and calls their respective methods to perform the operations.
